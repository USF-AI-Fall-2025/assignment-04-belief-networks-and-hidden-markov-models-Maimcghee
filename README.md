...
* Correct/typed pairs that are identical after lowercasing (ie. Presbyterian/presbyterian) are kept as valid training examples, they teach the emission table that typing a letter correctly is the common case.
...
The full HMM barely outperformed the baseline on exact word accuracy. Both scored around 2.3% at threshold=0, since the test set contains only one word, the model corrected exactly right: liquify and liquuf to liquefy. At higher thresholds, the HMM's exact word accuracy actually matched the baseline exactly at 2.3% because it stopped attempting any corrections that would count as successful, while character accuracy improved slightly 76.7% to 78.4%, as the threshold rose, since the model made fewer risky wrong guesses. The successful correction rate was 2.4% at threshold=0 (1 of 42 misspelled test words) and 0% at every higher threshold tested, since that one success's improvement score: ~0.85–1.18 didn't clear thresholds of 1 or above.
...
Real typos would include far more insertions and deletions than this datasets substitution only typos, this model cannot attempt those at all. So real world accuracy would likely look considerably worse than these numbers unless paired with a more general alignment based model that can handle typed and intended words of different lengths.

## AI Assistance: 

I used Claud AI to help fill some gaps, I tried not to let it give me the answeres and instead rewatched parts of lecture videos to fill details of thoes were missing. for the most part I asked it to remind me of the structure of how to do certain things and for the Viterbi decoding I asked it to give me the steps and it gave me a bit more detail then the spec did. I also asked it to remind me about smoothing and I used its responce to work out the equasion. For everything else I tried to implement it on my own, testing and debugging as needed. 

Below you can find the link to the transcript of the conversation: 

https://claude.ai/share/7d5774a1-ebd3-42bf-8b8f-9139b354ed30